...
class Peak(object):
    ...
    def __cal_read_count(self, reads_pos, ext):
        ...
        """
        if self.chrm not in list(reads_pos.keys()):
            return 0
...
def get_common_peaks(pks1, pks2):
    ...
    common_chrm = set(pks1.keys()).intersection(list(pks2.keys()))
...
def merge_common_peaks(pks1_common, pks2_common):
    ...
    merged_pks = {}
    summit_dist = {}
    for key in list(pks1_common.keys()):
...
def _add_peaks(pks1, pks2):
    ...
    for key in keys:
        value = []
        if key in list(pks1.keys()):
            value += pks1[key]
        if key in list(pks2.keys()):
            value += pks2[key]
        peaks[key] = value
    return peaks
